        ...
        private static void Main(string[] args)
        {
            ...
            using (var toCorlib = AssemblyDefinition.ReadAssembly("to_mscorlib.dll"))
            {
                ...
                foreach (var diffTypePair in diffTypes)
                {
                    ...
                    foreach (var originalTypeInterface in originalType.Interfaces)
                    {
                        var imp = new InterfaceImplementation(
                            diff.ResolveType(originalTypeInterface.InterfaceType));
                    ...
                    if (methodsToInclude.TryGetValue(td.FullName, out var methods))
                        foreach (var method in methods)
                        {
                            ...
                            {
                                var modRef =
                                    diff.ModuleReferences.FirstOrDefault(
                                        m => m.Name == method.PInvokeInfo.Module.Name);
                                if (modRef == null)
                                {
                                    modRef = new ModuleReference(method.PInvokeInfo.Module.Name);
                                    diff.ModuleReferences.Add(modRef);
                                }

                                md.PInvokeInfo = new PInvokeInfo(method.PInvokeInfo.Attributes, method.PInvokeInfo.EntryPoint, modRef);
                            }
                    ...
                    foreach (var md in td.Methods)
                    {
                        ...
                        if (originalMethod.Body.HasExceptionHandlers)
                            foreach (var bodyExceptionHandler in originalMethod.Body.ExceptionHandlers)
                            {
                                ...
                                exHandler.CatchType = bodyExceptionHandler.CatchType != null ? diff.ResolveType(bodyExceptionHandler.CatchType) : null;

                                if (bodyExceptionHandler.TryStart != null)
                                    exHandler.TryStart =
                                        md.Body.Instructions[originalMethod.Body.Instructions.IndexOf(bodyExceptionHandler.TryStart)];

                                if (bodyExceptionHandler.TryEnd != null)
                                    exHandler.TryEnd =
                                        md.Body.Instructions[originalMethod.Body.Instructions.IndexOf(bodyExceptionHandler.TryEnd)];

                                if (bodyExceptionHandler.FilterStart != null)
                                    exHandler.FilterStart =
                                        md.Body.Instructions[originalMethod.Body.Instructions.IndexOf(bodyExceptionHandler.FilterStart)];

                                if (bodyExceptionHandler.HandlerStart != null)
                                    exHandler.HandlerStart =
                                        md.Body.Instructions[originalMethod.Body.Instructions.IndexOf(bodyExceptionHandler.HandlerStart)];

                                if (bodyExceptionHandler.HandlerEnd != null)
                                    exHandler.HandlerEnd =
                                        md.Body.Instructions[originalMethod.Body.Instructions.IndexOf(bodyExceptionHandler.HandlerEnd)];
                ...
                foreach (var diffTypePair in diffTypes)
                {
                    ...
                    foreach (var originalProperty in originalType.Properties)
                    {
                        var pd = new PropertyDefinition(originalProperty.Name, originalProperty.Attributes,
                                                        diff.ResolveType(originalProperty.PropertyType));

                        if (diffMethods.TryGetValue(originalProperty.GetMethod?.FullName ?? "", out var newGet))
                            pd.GetMethod = newGet;
                        if (diffMethods.TryGetValue(originalProperty.SetMethod?.FullName ?? "", out var newSet))
                            pd.SetMethod = newSet;
